...
(function(exports) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      chrome.sockets.tcp.onReceive.addListener(function(info) {
        if(i===0) {
          ...
          max = Math.ceil(responseLength / (1024 * 4));
          console.log('max: '+max);
        }
        console.log(i);
        //TODO add info.data to a bigger buffer
        if(max < 2 || i === max) {
          resolve(info);
        }
        i++;
  ...
  var search = function(q, callback) {
    ...
    setTimeout(function() { 
      sendToServer2($.getSearchResultRequest()).then(function (receiveInfo) {
