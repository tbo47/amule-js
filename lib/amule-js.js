...
(function(exports) {
  ...
  $.readBufferChildren = function(buffer, resChild) {
    ...
    for (var j = 0; j < resChild.tagCountInResponse; j++) {
      ...
      if(child.nameEcTag % 2 && child.nameEcTag>1) {
        ...
        //$.readBufferChildren(buffer, child);
        // TODO lire la fin du child buffer sinon ca va etre decale
        child.value = readBuffer(buffer, child.length);
      } else {
      }
    }
    return resChild;
  ...
  $.readResultsList = function(buffer) {
    ...
    l("response length (total minus header and response Length): " + res.responseLength);
