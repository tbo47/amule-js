...
(function(exports) {
  ...
  var readSalt = function(buffer) {
    ...
    // var dataView = new DataView(buffer, offset,
    // Uint16Array.BYTES_PER_ELEMENT);
    // console.log("response tag # name (ecTag) : " + dataView.getUint16(0,
    // false));
    offset = offset + Uint16Array.BYTES_PER_ELEMENT;

    // var dataView = new DataView(buffer, offset,
    // Uint8Array.BYTES_PER_ELEMENT);
    // console.log("response tag # type (ecOp): " + dataView.getUint8(0,
    // false));
    offset = offset + Uint8Array.BYTES_PER_ELEMENT;

    // var dataView = new DataView(buffer, offset,
    // Uint32Array.BYTES_PER_ELEMENT);
    // console.log("response tag # length : " + dataView.getUint32(0,
    // false));
    offset = offset + Uint32Array.BYTES_PER_ELEMENT;
   ...
   *
   */
  $.readBufferChildren = function(buffer, resChild, totalSizeOfRequest) {
    resChild.children = [];
  ...
  $.readResultsList = function(buffer) {
    ...
    $.readBufferChildren(buffer, res[jsonChild], res.totalSizeOfRequest);
    console.log(res);
    return res;
  };
  ...
  var sendToServer2 = function (data) {
    return new Promise(function(resolve, reject) {
      console.log('sendToServer --------------------------------------------------------');
      var index = 0, effectiveSize = 0, expectedLength = 0, buf = [];
      chrome.sockets.tcp.send(socketId, data, function(sendInfo) {});
      chrome.sockets.tcp.onReceive.addListener(function(info) {
        ...
        if(c===0) {
          ...
          readBuffer(info.data, 4);// header
          var responseLength = readBuffer(info.data, 4);
          console.log('responseLength: ' + responseLength);
          max = Math.floor(responseLength / (1024 * 4));
          console.log('max: '+max);
        }
        if(max === 0) {
          console.log('resolve --------------------------------------------------------');
          ...
          arrayBuffers.push(info.data);
          if(c === max) {
            var bufferLength = 0;
            for (var i = 0; i < arrayBuffers.length; i++) {
              bufferLength = bufferLength + arrayBuffers[i].byteLength;
            }
            var buffer = new ArrayBuffer(bufferLength);
            var offset = 0;
            for (var i = 0; i < arrayBuffers.length; i++) {
              for (var j = 0; j < arrayBuffers[i].byteLength; j++) {
                var fromArrayView = new Uint8Array(arrayBuffers[i], j, 1);
                var toArrayView = new Uint8Array(buffer, j + offset, 1);
                toArrayView.set(fromArrayView);
              }
              offset = offset + arrayBuffers[i].byteLength;
            }
            arrayBuffers = [];
            info.data = buffer;
            resolve(info);
  ...
  var connect = function(ip, port, password, md5, callback) {
    initConnToServer(ip, port).then(function() {
      $.init(password, md5);
      return sendToServer($.getAuthRequest1());
    })
    .then(function(receiveInfo) {
      readSalt(receiveInfo.data);
      return sendToServer($.getAuthRequest2());
    })
    .then(function(receiveInfo) {
      if (readSalt(receiveInfo.data) === 4) {
  ...
  var fetchSearch = function(callback) {
    sendToServer($.getSearchResultRequest()).then(function (receiveInfo) {
      callback($.readResultsList(receiveInfo.data));
    });
  };
